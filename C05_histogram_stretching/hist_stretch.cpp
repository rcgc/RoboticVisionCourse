...
void userdefined ()
{
	...
	int col, row;
	// Factor de excalamiento
	float ScaleFactor;
	// Mínimo y máximo valor de pixel de actual 
	int min = 255;
	int max = 0;
	
	// 1.- Guardar la imagen en una matriz
	...
	printf("Pixel valor mínimo: %d\n", min);
	printf("Pixel valor máximo: %d\n", max);
	
	// 3.- Obtener el factor de escalamiento
	ScaleFactor = 255.0/(max-min);
	
	// 4.- Estirar el histograma
	for(row = 0; row < 512; row++){
		for(col = 0; col < 512; col++){
			Pixel = mat[col][row];
