...
void openfiles ()
{
     ...
     printf(" Enter name of *.xls OUTPUT histogram file (example: histogram.xls) ");
     scanf("%s",&outfile);
     
     //Check if output file was created succesfully
     if ((outfptrh = fopen(outfile, "wa")) == NULL)
     {
       printf(" Cannot create xls file: %s\n",outfile);
       printf(" Exiting program...\n");
       system("PAUSE");
       exit(1);
       }
...
void userdefined ()
{
	...
	// Inicializar el vector, ceros
	for(int i=0; i<256; i++){
		histcount[i]=0;
	}
	
	// Contar valores pixeles y guardarlos
	Pixel = fgetc(infptr);
	do{
		histcount[Pixel]++;
		Pixel = fgetc(infptr);
	...
	// Convertir los valores de enteros a string para la lectura de excel
	for(int i=0; i<256; i++){
		itoa(histcount[i], string, 10);
		
		// Guardar en archivo de salida
		fputs(string, outfptrh);
		// Inicio de delimitador para CSV
		fputs("\n", outfptrh);
	}
